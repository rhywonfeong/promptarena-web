...
- **CF Worker 的 fetch 出口跟随访问者接入舱位**（香港用户落 HKG → 出口判 HK → 仍 403，实测）：代理链路必须落固定出口 —— Worker → `or.collectui.pro`（**oh1** 美国服务器 caddy 透传 `:8789`，ul-mirror-oh1 tunnel 接入；2026-08-25 自短期机 va1 迁来，va1 将弃用）→ openrouter.ai；**CF 注入的身份地理头（CF-IPCountry/CF-Connecting-IP 等）沿链路层层传染**，worker 与 caddy 两跳都要剥（`worker/index.ts` STRIP_HEADERS + oh1 `/home/ubuntu/app/or-proxy/Caddyfile` 的 header_up -CF-*）；出口密钥 `X-Proxy-Key` = wrangler secret `PROXY_KEY` = oh1 docker env = `ul-mirror/.env` 的 `OR_PROXY_KEY`
